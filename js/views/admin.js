...
export function initAdminView() {
    ...
    if(dom.adminTabUsers) dom.adminTabUsers.onclick = () => switchAdminTab('users');
    if(dom.adminTabBookings) dom.adminTabBookings.onclick = () => switchAdminTab('bookings');
}
...
export async function loadAdminData() {
    ...
    if(dom.adminTopList) renderTopList(bookings);
    renderUsersList(users);
    renderBookingsList(bookings);
}

function renderStats(users, bookings) {
    if(dom.statUsers) dom.statUsers.textContent = users.length;
    const now = new Date().toISOString();
    const active = bookings.filter(b => b.endZeit > now).length;
    if(dom.statBookings) dom.statBookings.textContent = active;
    const p1Count = bookings.filter(b => b.parkplatzId === 'P1').length;
    const p2Count = bookings.filter(b => b.parkplatzId === 'P2').length;
    if(dom.statTopSpot) dom.statTopSpot.textContent = p1Count >= p2Count ? 'P1' : 'P2';
}
...
function renderTopList(bookings) {
    ...
    sorted.forEach((item, index) => {
        const name = item[0];
        const count = item[1];
        html += `<div class="top-list-item"><div class="top-rank">${medals[index] || (index + 1) + '.'}</div><div class="top-name">${name}</div><div class="top-count">${count} Buchungen</div></div>`;
    });
...
function renderUsersList(users) {
    ...
    users.forEach(u => {
        ...
        html += `<div class="user-list-item"><div class="user-info"><h4>${u.partei}</h4><p>${u.email}</p>${isBlocked ? '<small style="color:var(--danger)">GESPERRT</small>' : ''}</div><div class="user-actions"><button class="action-btn-icon btn-block ${blockClass}" onclick="toggleBlockUser('${u.id}', ${!isBlocked})"><i class="fa-solid ${blockIcon}"></i></button><button class="action-btn-icon btn-delete" onclick="deleteUserEntry('${u.id}')"><i class="fa-solid fa-trash"></i></button></div></div>`;
    });
    ...
    window.toggleBlockUser = async (uid, blockStatus) => {
        if(confirm(blockStatus ? "Sperren?" : "Entsperren?")) { await updateDoc(doc(db, "users", uid), { isBlocked: blockStatus }); loadAdminData(); }
    };
    window.deleteUserEntry = async (uid) => {
        if(confirm("Löschen?")) { await deleteDoc(doc(db, "users", uid)); loadAdminData(); }
    };
}
...
function renderBookingsList(bookings) {
    ...
    bookings.forEach(b => {
        ...
        html += `<div class="booking-item" style="background:var(--input-bg); border:none;"><div><strong>${b.parkplatzId}</strong>: ${start}<br><span class="small-text">${b.partei} (${b.kennzeichen || '?'})</span></div><button class="action-btn-icon btn-delete" onclick="deleteBookingAdmin('${b.id}')"><i class="fa-solid fa-xmark"></i></button></div>`;
    });
    dom.adminBookingList.innerHTML = html || '<p class="text-center small-text">Keine Buchungen.</p>';
    window.deleteBookingAdmin = async (bid) => {
        if(confirm("Löschen?")) { await deleteDoc(doc(db, "bookings", bid)); loadAdminData(); }
    };
}